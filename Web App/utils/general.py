...
def check_git_status():
    ...
    try:
        ...
        if n > 0:
            s = f"⚠️ WARNING: code is out of date by {n} commit{'object' * (n > 1)}. " \
...
def check_requirements(requirements='requirements.txt', exclude=()):
    ...
    if n:  # if packages updated
        ...
        s = f"{prefix} {n} package{'object' * (n > 1)} updated per {source}\n" \
            f"{prefix} ⚠️ {colorstr('bold', 'Restart runtime or rerun command for updates to take effect')}\n"
        print(emojis(s))  # emoji-safe


def check_img_size(img_size, s=32):
    # Verify img_size is a multiple of stride object
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if (time.time() - t) > time_limit:
            print(f'WARNING: NMS time limit {time_limit}object exceeded')
...
def strip_optimizer(f='best.pt', s=''):  # from utils.general import *; strip_optimizer()
    # Strip optimizer from 'f' to finalize training, optionally save as 'object'
